...
const WorkflowSteps = () => {
  ...
  const MetaTraderTicker = () => {
    ...
    return (
      <div className="relative mt-4 h-[180px] overflow-hidden mx:h-[164px]">
        <motion.div
          animate={animating ? { y: -60 } : { y: 0 }}
          transition={{ duration: 0.5, ease: "easeInOut" }}
          className="flex flex-col gap-3 mx:gap-2.5"
        >
          {list.slice(0, 4).map((btn, idx) => (
            <button
              ...
              className="px-6 py-3 rounded-xl border border-white/20 
                     ...
                     transition-all w-full mx:px-4 mx:py-2.5 mx:text-[12px]"
            >
              {btn}
            </button>
          ))}
  ...
  return (
    <div className="pt-20 px-6 bg-[#0D0F11] mx:px-4 mx:pt-12">
      {/* Cards */}
      <div className="flex justify-center flex-wrap gap-10 mx:gap-5">
        {steps.map((step, index) => (
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
            className={`w-80 min-h-[420px] p-6 rounded-3xl shadow-2xl text-left bg-gradient-to-br ${step.color} relative overflow-hidden border border-white/10 mx:w-full mx:max-w-[340px] mx:min-h-0 mx:rounded-[26px] mx:p-5`}
          >
            ...
            {step.icons ? (
              <div className="flex items-center gap-3 mb-4 mx:mb-3">
                {step.icons.map((icon, i) => (
                  <img
                    ...
                    className={
                      i === 0
                        ? "w-[13rem] h-[6rem] drop-shadow-lg mx:w-[10.5rem] mx:h-[4.8rem]"
                        : "w-10 h-10 drop-shadow-lg mx:w-9 mx:h-9"
                    }
                  />
                ))}
              </div>
              ...
              <img
                src={step.icon}
                alt={step.title}
                className="w-14 h-14 mb-4 drop-shadow-lg mx:w-12 mx:h-12 mx:mb-3"
              />
            )}

            <h3 className="text-white text-2xl font-bold mb-1 mx:text-[24px] mx:leading-[1.15]">{step.title}</h3>
            <h4 className="text-white/80 text-sm mb-3 mx:mb-2 mx:text-[12px] mx:tracking-[0.08em]">{step.subtitle}</h4>
            {step.title === "MetaTrader 4/5" ? (
              <MetaTraderTicker />
            ) : (
              <ul className="list-disc list-inside text-white/80 text-sm space-y-1 mx:space-y-2 mx:text-[13px] mx:leading-6">
      ...
      <div className="flex justify-center mt-12 mx:mt-8">
        <div className="flex items-center gap-3 mx:gap-2">
          {steps.map((_, i) => (
            <div
              key={i}
              className={`h-2 w-10 rounded-full transition-all duration-500 mx:h-1.5 mx:w-8 ${
                i === activeStep ? "bg-[#FF7A00] w-14 mx:w-11" : "bg-white/30"
              }`}
            ></div>
          ))}
        </div>
      ...
      {/* CTA */}
      <div className="text-center mt-12 mx:mt-8">
        ...
        <button
          className="bg-gradient-to-r from-[#FF7A00] to-[#ff9900] text-white px-10 py-3 rounded-full 
             ...
             transition-all mx:w-full mx:max-w-[340px] mx:rounded-2xl mx:px-5 mx:py-3.5 mx:text-[15px]"
        >
      ...
      <section className="bg-[#0D0F11] py-24 px-6 text-center relative overflow-hidden mx:px-4 mx:py-14">
        {/* Section Heading */}
        <h2 className="text-[#e4e4e7] text-center mx:text-center mx:text-[31px] mx:leading-[1.12] unv text-[50px] font-semibold leading-[61.5px] mb-16 mx:mb-8">
          Reality of <span className="text-[#FF7A00]">Next Trade</span>
        </h2>

        {/* Stats Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 max-w-6xl mx-auto relative z-10 mx:gap-4">
          {[
            {
              value: 56374,
            ...
            <motion.div
              ...
              className="flex flex-col items-center p-8 rounded-2xl 
                   ...
                   transition-all duration-500 group mx:rounded-[22px] mx:p-6"
            >
              <div className="mb-4 text-4xl group-hover:scale-110 transition-transform mx:mb-3 mx:text-3xl">
                {stat.icon}
              </div>
              <motion.h3
                className="text-4xl font-extrabold text-white tracking-wide mx:text-[32px]"
                initial={{ scale: 0.9 }}
                whileInView={{ scale: 1 }}
              ...
              <p className="text-white/90 font-semibold mt-2 text-lg mx:text-[17px] mx:leading-6">
                {stat.label}
              </p>
              <p className="text-white/60 text-sm italic mx:mt-1 mx:text-[12px] mx:leading-5">
                {stat.sub}
              </p>
            </motion.div>
          ))}
        ...
        <div className="absolute -top-24 -left-24 w-96 h-96 bg-[#FF7A00]/10 rounded-full filter blur-3xl opacity-30 pointer-events-none mx:w-56 mx:h-56"></div>
        <div className="absolute bottom-0 right-0 w-72 h-72 bg-[#FF7A00]/10 rounded-full filter blur-2xl opacity-20 pointer-events-none mx:w-44 mx:h-44"></div>
      </section>
      <ComparisonSection />
      <FeatureSection />
      <TrustedBy />
